...
const userColumns: Column<UserRow>[] = [
  {
    ...
    render: (row) => (
      <div className="flex items-center gap-3">
        <div className="rounded-2xl bg-white/15 p-2.5 text-[color:var(--color-foreground)] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]">
          <ShieldCheck size={16} />
        </div>
        <div>
          <Link
            href={`/admin/users/${encodeURIComponent(row.id)}`}
            className="font-semibold text-[color:var(--color-foreground)] hover:opacity-80"
          >
            {row.full_name}
          </Link>
  ...
  },
  {
    ...
    sortable: true,
    render: (row) => (
      <span className="text-[color:var(--color-muted)] font-mono text-xs">{row.phone_number}</span>
    ),
  },
  {
    key: "wallet",
    ...
    render: (row) => {
      ...
      if (!wallet) return <span className="text-[color:var(--color-muted)]">-</span>;
      return (
        <span className="inline-flex items-center rounded-full bg-white/15 px-3 py-1 text-xs font-semibold text-[color:var(--color-foreground)] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]">
...
export default function AdminUsersPage() {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-6">
      ...
      >
        <h1 className="text-2xl font-extrabold tracking-tight text-[color:var(--color-foreground)]">Users</h1>
        <p className="mt-1 text-sm font-medium text-[color:var(--color-muted)]">Search users, review KYC, and approve instantly.</p>
      </motion.div>

      <motion.div variants={itemVariants} className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-[1.75rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.10)] backdrop-blur-xl">
          <div className="text-xs font-semibold uppercase tracking-wider text-[color:var(--color-muted)]">Total Users</div>
          <div className="mt-2 text-2xl font-extrabold text-[color:var(--color-foreground)]">{stats.total}</div>
        </div>
        <div className="rounded-[1.75rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.10)] backdrop-blur-xl">
          <div className="text-xs font-semibold uppercase tracking-wider text-[color:var(--color-muted)]">KYC Pending</div>
          <div className="mt-2 text-2xl font-extrabold text-[color:var(--color-foreground)]">{stats.pending}</div>
        </div>
        <div className="rounded-[1.75rem] border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.10)] backdrop-blur-xl">
          <div className="text-xs font-semibold uppercase tracking-wider text-[color:var(--color-muted)]">KYC Approved</div>
          <div className="mt-2 text-2xl font-extrabold text-[color:var(--color-foreground)]">{stats.approved}</div>
        </div>
      </motion.div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.10)]">
            <h2 className="text-xl font-extrabold text-[color:var(--color-foreground)]">Create User</h2>
      ...
      {showEditModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-[color:var(--color-border)] bg-[color:var(--color-surface)] p-6 shadow-[0_18px_55px_rgba(0,0,0,0.10)]">
            <h2 className="text-xl font-extrabold text-[color:var(--color-foreground)]">Edit User</h2>
            <div className="mt-4 space-y-4">
              <input
                value={formData.full_name}
                onChange={(e) => setFormData({ ...formData, full_name: e.target.value })}
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded"
                />
                <label htmlFor="pep" className="text-sm text-[color:var(--color-foreground)]">PEP Status</label>
              </div>
            </div>
            <div className="mt-6 flex gap-3">
              <button
